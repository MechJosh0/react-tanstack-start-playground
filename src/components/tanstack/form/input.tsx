...
type FieldLike = FieldApi<any, any, any, any, any, any, any, any, any, any, any, any, any, any, any, any, any, any, any, any, any, any, any>;

export interface InputProps extends Omit<React.ComponentPropsWithoutRef<'input'>, 'id' | 'name' | 'value' | 'onChange' | 'onBlur'> {
  field: FieldLike;
  label?: string;
  help?: string;
  parse?: (v: string) => any;
...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(function Input({ field, label, help, parse, format, type = 'text', wrapperClassName, inputClassName, labelClassName, className, ...props }, ref) {
  ...
  return (
    <div className={cn('space-y-2', wrapperClassName, className)}>
      {label && (
        <Label htmlFor={field.name} className={cn('text-zinc-300', labelClassName)}>
          {label}
        </Label>
      )}
      ...
      <UIInput
        ...
        onChange={(e) => field.handleChange(parse ? parse(e.currentTarget.value) : e.currentTarget.value)}
        aria-invalid={hasError || undefined}
        aria-describedby={hasError ? errId : help ? helpId : undefined}
        className={cn('bg-zinc-800/70 border-zinc-700 text-zinc-200 placeholder-zinc-500', 'focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2 ring-offset-[#0b0d12]', inputClassName)}
        {...props}
      />
